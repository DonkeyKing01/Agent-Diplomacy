...
async def check_database_health() -> bool:
    ...
    try:
        # Self-heal: when MGX_IGNORE_INIT_DB is set, initialize_database() skips
        # engine/session-maker creation, so async_session_maker stays None until
        # the first get_db() request. The health endpoint does NOT go through
        # get_db(), so without this lazy init the probe would report unhealthy
        # forever and block startup. ensure_initialized() is idempotent.
        if not db_manager.async_session_maker:
            try:
                await db_manager.ensure_initialized()
            except Exception as e:
                logger.error(f"Health check lazy initialization failed: {e}")
                return False
