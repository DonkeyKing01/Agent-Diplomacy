...
import { fetchPortalSnapshotBundle, PortalSnapshotBundle, fetchSpectatorPrivate, SpectatorPrivateState } from '@/game/api';
...
const PlayerPortalPage: React.FC = () => {
  ...
  const unlock = async (nextPassword?: string) => {
    ...
    setLoading(true);
    try {
      const result = await fetchSpectatorPrivate({ nation_id: nationId, password: usingPassword });
  ...
  useEffect(() => {
    ...
    if (!cached) return;
    unlock(cached);
    const timer = window.setInterval(() => unlock(cached), 15000);
        ...
        <section className="rounded-xl border border-border bg-card px-5 py-4">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="rounded-full border border-primary/35 bg-primary/10 px-3 py-1.5 text-sm text-primary">
              {mode === 'live' ? '实时模式 · 15 秒自动刷新' : '快照模式 · 15 秒轮询私有快照'}
            </div>
          </div>
        </section>
                ...
                data.blackbox.decision_replay.entries.map((entry, index) => (
                  <div key={`${entry.timestamp}-${index}`} className="rounded-lg border border-border/70 bg-background/40 p-4">
                    ...
                    {(entry.reasoning_trace?.risks || []).length > 0 ? (
                      <div className="mt-2 text-sm text-muted-foreground">
                        Risks：{entry.reasoning_trace?.risks?.join('；')}
                      </div>
                    ) : null}
                  </div>
                ))
